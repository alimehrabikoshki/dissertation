...
outputular=sprintf("iperf_udp_20stream_%s_packetloss.png", ARG1)
path=sprintf("../../results/results/png/%s/", ARG1)
outputular=sprintf("%s%s", path, outputular)
set output outputular
titular=sprintf("iPerf3 UDP Packet Loss - 20 session - %s", ARG1)
set title titular
set size 1,1
set grid y
...
filename=sprintf("../../results/results/tsv/%s/iperf_udp_20stream_packetloss_stats.tsv", ARG1)
