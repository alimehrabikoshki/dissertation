...
outputular=sprintf("apache_200K_500_%s_percentiles.png", ARG1)
path=sprintf("../../results/results/png/%s/", ARG1)
outputular=sprintf("%s%s", path, outputular)
set output outputular
titular=sprintf("ApacheBench HTTP Response time percentiles - 200K Requests-  500 session - %s", ARG1)
set title titular
set size 1,0.8
set grid y
set xlabel "Time (s)"
...
filename=sprintf("../../results/results/tsv/%s/apache_200K_500_percentile_stats.tsv", ARG1)
