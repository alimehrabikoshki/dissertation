...
do
  ...
  do
     echo "$(echo ${n} |  cut -d '-' -f1 | sed 's:.*/::')    $(grep "receiver" ${n} |  grep "SUM" |  awk -F " " '{print $11}'| awk -F "[()]" '{print $2}' |  cut -d '%' -f1 | xargs printf "%.6f")" >> ../../results/results/tsv/${testcase}/iperf_udp_20stream_packetloss_stats.tsv
  done
done