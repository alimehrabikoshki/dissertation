...
outputular=sprintf("iperf_udp_1stream_%s_packetspersecond.png", ARG1)
path=sprintf("../../results/results/png/%s/", ARG1)
outputular=sprintf("%s%s", path, outputular)
set output outputular
titular=sprintf("iPerf3 UDP Packets per second - 1 session - %s", ARG1)
set title titular
set size 1,0.6
set grid y
set xlabel "Time (s)"
...
filename=sprintf("../../results/results/tsv/%s/iperf_udp_1stream_pps_stats.tsv", ARG1)
